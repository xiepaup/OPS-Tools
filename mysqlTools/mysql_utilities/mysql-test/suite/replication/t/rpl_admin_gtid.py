...
_DEFAULT_MYSQL_OPTS_FILE = '"--log-bin=mysql-bin --skip-subordinate-start ' + \
                           ...
                           '--sync-main-info=1 --main-info-repository=file"'
...
class test(rpl_admin.test):
    ...
    def run(self):
        ...
        cmd_opts = " --candidates=%s  " % subordinates
        cmd_opts += " --subordinates=%s elect" % subordinates
        ...
        subordinates = ",".join([subordinate1_conn, subordinate2_conn, subordinate3_conn])
        cmd_str = "mysqlrpladmin.py --main=%s " % main_conn
        cmd_opts = " --subordinates=%s gtid --format=csv " % subordinates
        res = mutlib.System_test.run_test_case(self, 0, cmd_str+cmd_opts,
                                               comment)
        if not res:
            raise MUTLibError("%s: failed" % comment)
        ...
        subordinates = ",".join([subordinate1_conn, subordinate2_conn, subordinate3_conn])
        cmd_str = "mysqlrpladmin.py --main=%s " % main_conn
        cmd_opts = " --discover-subordinates-login=root:root health --format=csv "
        res = mutlib.System_test.run_test_case(self, 0, cmd_str+cmd_opts,
                                               comment)
        if not res:
            raise MUTLibError("%s: failed" % comment)
        ...
        cmd_str = "mysqlrpladmin.py --main=%s " % main_conn
        cmd_opts = " --candidates=%s  " % subordinate3_conn
        cmd_opts += " --subordinates=%s failover" % subordinates
        res = mutlib.System_test.run_test_case(self, 0, cmd_str+cmd_opts,
                                               comment)
        if not res:
            raise MUTLibError("%s: failed" % comment)
        ...
        subordinates = ",".join([subordinate1_conn, subordinate2_conn])
        cmd_str = "mysqlrpladmin.py --main=%s " % subordinate3_conn
        comment = "Test case %s - show health after failover" % test_num
        cmd_opts = " --subordinates=%s --format=vertical health" % subordinates
        res = mutlib.System_test.run_test_case(self, 0, cmd_str+cmd_opts,
                                               comment)
        if not res:
            raise MUTLibError("%s: failed" % comment)
        ...
        cmd_opts = " --subordinates=%s elect -vvv --candidates=%s " % \
                   (subordinates, subordinate1_conn)
        res = mutlib.System_test.run_test_case(self, 0, cmd_str+cmd_opts,
                                               comment)
        if not res:
            raise MUTLibError("%s: failed" % comment)
        ...
        subordinates = ",".join([subordinate1_conn, subordinate2_conn, subordinate3_conn, subordinate4_conn])
        cmd_str = "mysqlrpladmin.py --main=%s " % main_conn
        cmd_opts = " --discover-subordinates-login=root:root switchover "
        cmd_opts += "--new-main=root:root@localhost:%s " % self.s4_port
