...
_DEFAULT_MYSQL_OPTS = ' '.join(['"--log-bin=mysql-bin --skip-subordinate-start',
                                '--log-subordinate-updates --gtid-mode=on',
                                '--enforce-gtid-consistency',
                                '--report-host=localhost',
                                '--report-port=%s --sync-main-info=1',
...
class test(rpl_admin.test):
    ...
    def setup(self):
        ...
        mysqld = _DEFAULT_MYSQL_OPTS % self.servers.view_next_port()
        self.server5 = self.spawn_server("rep_subordinate4_gtid", mysqld, True)
        ...
        self.main_conn = build_str(self.server1).strip(' ')
        self.subordinate1_conn = build_str(self.server2).strip(' ')
        self.subordinate2_conn = build_str(self.server3).strip(' ')
        self.subordinate3_conn = build_str(self.server4).strip(' ')
        self.subordinate4_conn = build_str(self.server5).strip(' ')
        ...
        cmd = " ".join(["mysqlreplicate.py --rpl-user=rpl:rpl ",
                        " --subordinate=%s" % self.subordinate4_conn,
                        " --main=%s" % self.main_conn])
    ...
    def wait_for_subordinate(self, main, subordinate):
        main_gtid = main.exec_query("SELECT @@GLOBAL.GTID_EXECUTED")
        main_gtids = main_gtid[0][0].split('\n')
        for gtid in main_gtids:
            try:
                res = subordinate.exec_query(_GTID_WAIT % (gtid.strip(','), 300))
    ...
    def run(self):
        ...
        # create a database then add data shutting down subordinates to create
        # a scenario where the candidate subordinate has fewer transactions than
        # the subordinates to execute the relay log execution logic and the
        # check for transactions to skip connecting candidate to subordinates as
        # its main
        ...
        self.wait_for_subordinate(self.server1, self.server2)
        self.wait_for_subordinate(self.server1, self.server3)

        # Stop the candidate and one other subordinate
        ...
        self.wait_for_subordinate(self.server1, self.server4)

        # Stop the third subordinate
        ...
        self.wait_for_subordinate(self.server1, self.server5)
        ...
        subordinates = ",".join([self.subordinate2_conn, self.subordinate3_conn,
                           self.subordinate4_conn])
        cmd_str = "mysqlrpladmin.py --main=%s " % self.main_conn
        cmd_opts = " --candidates=%s  " % self.subordinate1_conn
        cmd_opts += " --subordinates=%s failover -vvv" % subordinates
        ...
        for subordinate in [self.server3, self.server4, self.server5]:
            self.wait_for_subordinate(self.server2, subordinate)

        # Show contents of server
        for server in self.servers:
            self.dump_table(server)

        comment = ("Test case %s - failover to %s:%s with skipping subordinates" %
                   (test_num, self.server3.host, self.server3.port))
        subordinates = ",".join([self.subordinate3_conn, self.subordinate4_conn])
        cmd_str = ("mysqlrpladmin.py --main=%s --candidates=%s --subordinates=%s "
                   "failover -vvv" %
                   (self.subordinate1_conn, self.subordinate2_conn, subordinates))
