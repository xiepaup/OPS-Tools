...
parser.add_option("--new-main", action="store", dest="new_main", default=None,
                  type="string", help="connection information for the "
                  "subordinate to be used to replace the main for switchover, "
...
# At least one of the options --discover-subordinates-login or --subordinates is required.
if not opt.discover and not opt.subordinates:
    parser.error("One of these options is required to use the utility: "
                 "--discover-subordinates-login or --subordinates.")
...
if command == 'switchover' and (opt.new_main is None or opt.main is None):
    parser.error("The switchover command requires the --main and "
                 "--new-main options.")
    
if command in ['health', 'gtid'] and opt.discover is None and \
   (opt.subordinates is None or opt.main is None):
    parser.error("The health and gtid commands requires the --main and "
                 "--subordinates options.")

if command in ['start', 'stop', 'reset'] and \
   (not opt.subordinates or not opt.main):
    parser.error("The start, stop and reset commands require the --main "
                 "and --subordinates options.")

if command in ['elect', 'failover', 'start', 'stop', 'reset'] and \
    not opt.discover and not opt.subordinates:
...
if opt.new_main and command != 'switchover':
    print("WARNING: The --new-main option is used only with the "
          "switchover command.")
...
if (opt.candidates or opt.new_main) and command in ['stop', 'start', 'reset']:
    print("WARNING: The --new-main and --candidates options are not "
...
if opt.new_main:
    try:
        ...
        parser.error("New main connection values invalid: %s." % err)
    except UtilError:
        _, err, _ = sys.exc_info()
        parser.error("New main connection values invalid: %s." % err.errmsg)
else:
    new_main_val = None
