...
parser.add_option("--main-info-file", action="store", dest="main_info",
                  ...
                  help="the name of the main information file on the subordinate."
                       "default = 'main.info' read from the data directory."
                       " Note: this option requires that the utility run on "
                       "the subordinate with appropriate file read access to the "
                       "data directory.")

# Add --show-subordinate-status
parser.add_option("--show-subordinate-status", "-s", action="store_true",
                  dest="subordinate_status", default=False, help="show subordinate status")
...
try:
    m_values = parse_connection(opt.main)
except FormatError:
...
# Parse source connection values
try:
    s_values = parse_connection(opt.subordinate)
except FormatError:
    _, err, _ = sys.exc_info()
